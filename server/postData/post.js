// var admin = require("firebase-admin");

// var serviceAccount = require("../json/serviceAccountKey.json");

// admin.initializeApp({
//   credential: admin.credential.cert(serviceAccount),
//   databaseURL: "https://bj3d-56ec0-default-rtdb.firebaseio.com",
// });

// const db = admin.firestore();

// const tweetVoiceRef = db.collection("tweetVoice");

// const countryRef = tweetVoiceRef.doc("country").collection("tweets");
// const englishRef = tweetVoiceRef.doc("english").collection("tweets");
// const voiceRef = tweetVoiceRef.doc("voice").collection("tweets");

// countryRef
//   .get()
//   .then((snapshot) => {
//     snapshot.forEach((doc) => {
//       console.log("Country Tweet ID:", doc.id, "Data:", doc.data());
//     });
//   })
//   .catch((error) => {
//     console.error("Error fetching country tweets:", error);
//   });

// englishRef
//   .get()
//   .then((snapshot) => {
//     snapshot.forEach((doc) => {
//       console.log("English Tweet ID:", doc.id, "Data:", doc.data());
//     });
//   })
//   .catch((error) => {
//     console.error("Error fetching english tweets:", error);
//   });

// voiceRef
//   .get()
//   .then((snapshot) => {
//     snapshot.forEach((doc) => {
//       console.log("Voice Tweet ID:", doc.id, "Data:", doc.data());
//     });
//   })
//   .catch((error) => {
//     console.error("Error fetching voice tweets:", error);
//   });

const posts = [
  {
    title: "Post #1",
    description: "This is the first post",
    thumbnail:
      "https://images.unsplash.com/photo-1593642532400-2682810df593?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=750&q=80",
  },
  {
    title: "Post #2",
    description: "This is the second post",
    thumbnail:
      "https://images.unsplash.com/photo-1625034712314-7bd692b60ecb?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=750&q=80",
  },
  {
    title: "Post #3",
    description: "This is the third post",
    thumbnail:
      "https://images.unsplash.com/photo-1625034892070-6a3cc12edb42?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=766&q=80",
  },
];
module.exports.getPostById = (id) => posts[id - 1];
